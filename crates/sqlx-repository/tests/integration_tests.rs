...
async fn test_user_crud_operations() {
    ...
    let updated_user = repo.update(user.id, update_user).await.expect("Failed to update user");
    ...
    // Clean up
    repo.hard_delete(user.id).await.expect("Failed to delete user");
...
async fn test_soft_delete_operations() {
    ...
    // Soft delete
    let deleted = repo.delete(user.id).await.expect("Failed to soft delete user");
    assert!(deleted);
    
    // Should not be found in normal queries
    let found_user = repo.find_by_id(user.id).await.expect("Failed to query user");
    ...
    let deleted_results = repo.search(deleted_params).await.expect("Failed to search deleted users");
    ...
    let found_user = repo.find_by_id(user.id).await.expect("Failed to find restored user");
    assert!(found_user.is_some());
...
async fn test_search_and_filtering() {
    ...
    for user_data in users_data {
        let user = repo.create(user_data).await.expect("Failed to create test user");
    ...
    let search_results = repo.search(search_params).await.expect("Failed to search users");
    assert!(search_results.items.iter().any(|u| u.name.contains("Alice")));
    ...
    let page_results = repo.search(page_params).await.expect("Failed to paginate users");
    ...
    let sort_results = repo.search(sort_params).await.expect("Failed to sort users");
    ...
    for user in created_users {
        repo.hard_delete(user.id).await.expect("Failed to delete test user");
    }
    cleanup_test_data(repo.pool()).await;
}
...
async fn test_post_crud_without_soft_delete() {
    ...
    let user = user_repo.create(create_user).await.expect("Failed to create user");
    ...
    let post = post_repo.create(create_post).await.expect("Failed to create post");
    ...
    let updated_post = post_repo.update(post.id, update_post).await.expect("Failed to update post");
    ...
    let deleted = post_repo.delete(post.id).await.expect("Failed to delete post");
    assert!(deleted);
    
    // Should not be found
    let found_post = post_repo.find_by_id(post.id).await.expect("Failed to query post");
    assert!(found_post.is_none());
    
    // Clean up user
    user_repo.hard_delete(user.id).await.expect("Failed to delete user");
...
async fn test_count_operations() {
    ...
    // Get initial count
    let initial_count = repo.count(SearchParams::default()).await.expect("Failed to get initial count");
    ...
    let new_count = repo.count(SearchParams::default()).await.expect("Failed to get new count");
    ...
    let filtered_count = repo.count(filter_params).await.expect("Failed to get filtered count");
