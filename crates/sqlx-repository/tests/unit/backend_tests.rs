...
fn test_postgres_convert_type() {
    ...
    assert_eq!(PostgresBackend::convert_type("DateTime<Utc>"), "TIMESTAMP WITH TIME ZONE");
...
fn test_postgres_build_select_query() {
    ...
    assert_eq!(query, "SELECT id, name FROM users WHERE status = $1 LIMIT 10 OFFSET 20");
}
...
fn test_postgres_build_insert_query() {
    ...
    assert_eq!(query, "INSERT INTO users (name, email) VALUES ($1, $2) RETURNING *");
...
fn test_postgres_build_update_query() {
    ...
    assert_eq!(query, "UPDATE users SET name = $1, email = $2 WHERE id = $3");
    
    // Update with returning
    let query = PostgresBackend::build_update_query("users", &["name", "email"], true);
    assert_eq!(query, "UPDATE users SET name = $1, email = $2 WHERE id = $3 RETURNING *");
    
    // Single column update
    let query = PostgresBackend::build_update_query("users", &["name"], true);
    assert_eq!(query, "UPDATE users SET name = $1 WHERE id = $2 RETURNING *");
}
...
fn test_postgres_query_building_consistency() {
    ...
    let select_query = PostgresBackend::build_select_query(
        "users", 
        &[], 
        &["name = $1", "email = $2"], 
        None, 
